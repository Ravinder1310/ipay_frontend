...
const Recharge = () => {
    ...
    const handleRecharge = async () => {
        ...
        if (!operator || !operator.rechargeCode) {
            console.error(`Invalid operator selected for recharge: ${selectedOperator}`);
            toast("Invalid operator selected.", {
                duration: 4000,
                position: 'top-center',
                style: {
            ...
            <div className='w-full md:w-[80%] lg:w-[60%] m-auto'>
                ...
                {!rechargeResult ? (
                    <>
                        ...
                        {!selectedPlan && (
                            <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-10 max-h-64 overflow-y-auto">
                                <ul>
                                    {plans.length > 0 ? (
                                        plans.map((plan, index) => (
                                            ...
                                            >
                                                ...
                                                <p className="text-black font-bold">Type: {plan.talktime}</p>
                                                <p className="text-gray-500">{plan.description}</p>
                                                <button
                                                    className='mt-6 bg-blue-700 text-white p-2 rounded-xl'
                                                    onClick={() => {setSelectedPlan(plan); handleType(plan.description)}}
                                                >
                                                    Select Plan
                                                </button>
                                            </li>
                        ...
                        {selectedPlan && (
                            <div className="mt-6 bg-gray-200 p-6 rounded-lg shadow-lg">
                                <h3 className="text-lg font-bold text-center text-green-700 mb-4">Selected Plan</h3>
                                <p className="text-center text-gray-700 mb-2">Operator: {selectedPlan.operator_name}</p>
                                <p className="text-center text-gray-700 mb-2">Amount: ₹{selectedPlan.mrp}</p>
                                <p className="text-center text-gray-700 mb-2">Validity: {selectedPlan.validity} days</p>
                                <p className="text-center text-gray-700 mb-2">Talktime: {selectedPlan.talktime} minutes</p>
                                <p className="text-center text-gray-700 mb-2">Data Benefits: {selectedPlan.data} GB</p>
                                <p className="text-center text-gray-700">Description: {selectedPlan.description}</p>
                                <p className="text-center text-gray-700">Type: {type}</p>
                    ...
                    <div className="flex flex-col items-center justify-center bg-green-100 p-8 rounded shadow-md">
                        <h3 className="text-2xl font-bold text-green-700 mb-4">Recharge Successful!</h3>
